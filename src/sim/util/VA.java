...
public class VA
{
    ...
    public static Point3d center(Point3d p0,Point3d p)
    {
        return new Point3d(0.5*(p0.x+p.x),0.5*(p0.y+p.y),0.5*(p0.z+p.z));
    }

    public static Point3d symetric(Point3d org,Point3d p)
    {
     ...
     */
    public static double dist2(int x0,int y0,int x1,int y1)
    {
    ...
    /**
     * Returns distance between points p1 & p2 of graphics.Point3d class.
     */
    ...
    /**
     * Returns the square of the distance between points p1 & p2 of graphics.Point3d class.
     */
    public static double dist2(Point3d p1,Point3d p2)
    {
    ...
    public static void _resize(Point3d p0,Point3d p1,double sc)
    {
    ...
    /** Returns a new graphics.Point3d equal to sc*p1 */
    public static Point3d scale(Point3d p1,double sc)
    {
        return new Point3d(p1.x*sc, p1.y*sc,p1.z*sc);
    }

    /**  scales p1 to be equal to sc*p1 */
    public static void _scale(Point3d p1,double sc)
    {
        p1.x*=sc; p1.y*=sc;p1.z*=sc;
    }

    /** Returns a new graphics.Point3d equal p1+(dx,dy,dz) */
    public static Point3d translate(Point3d p1,double dx,double dy,double dz)
    {
        return new Point3d(p1.x+dx, p1.y+dy, p1.z+dz);
    }

    /** translates p1 by (dx,dy,dz) */
    public static void _translate(Point3d p1,double dx,double dy,double dz)
    {
    ...
    public static void _add(Point3d p1,Point3d p2)
    {
        p1.x+=p2.x;
        p1.y+=p2.y;
        p1.z+=p2.z;
    ...
    /** Modifies p1 so that p1=p1-p2 */
    public static void _subtract(Point3d p1,Point3d p2)
    {
        p1.x-=p2.x;
        p1.y-=p2.y;
        p1.z-=p2.z;
    ...
    public static double inner(Point3d p1,Point3d p2)
    {
        return (p1.x*p2.x)+(p1.y*p2.y)+(p1.z*p2.z);
    }

    /** Returns |p1|. (norm -length- of p1) */
    public static void _normalize(Point3d p1)
    {
        double l=norm(p1);
        if (l!=0) { p1.x/=l; p1.y/=l; p1.z/=l;}
    }
    ...
    public static double cos(Point3d p1,Point3d p2)
    {
        ...
        if (l==0)
            ...
            return inner(p1,p2)/l ;
    }

    /** Projection of vector a on b. */
    public static Point3d proj(Point3d b, Point3d a)
    {
        double n=norm(b);
        if (n==0)
    ...
    /** Returns a graphics.Point3d equal to (p,0) */
    public static Point3d promote3d(Point p)
    {
    ...
    */
    ...
    static public Point3d normalize(Point3d p)
    {
        ...
        if (l==0)
            ...
            q=new Point3d(p.x/l,p.y/l,p.z/l);
        return q;
    }

    public static double norm(Point3d p1)
    {
    ...
    static public Point3d zap2Y(Point3d n)
    {
        ...
        else
        {
            ...
            if (Math.abs(Math.sin(Yang)-sinT)>0.0001)
                Log.println("===========> cosT,Yang:"+cosT+","+Yang);
            if (Math.abs(Math.cos(Yang)-cosT)>0.0001)
                Log.println("===========> sinT,Yang:"+sinT+","+Yang);
        }
        Point3d np=Yrotate(n,-Yang);

        if (Math.abs(np.x)>=0.0001)
            Log.println("------------> np:"+np.str()+"\n n:"+n.str());
        ...
        if (Math.abs(np.x)>=0.0001 || Math.abs(np.z)>=0.0001)
            Log.println("##############> np:"+np.str()+ "\n n:"+n.str());
    ...
    static public Point3d polar(double meridian, double parallel, double radius)
    {
    ...
    static public void addto(double M[], int m, double[] C)
    {
        for(int i=0; i<m; i++)
            M[i]+=C[i];
    }
    ...
    /**
     * Squash - Sigmoid function
     * we are using G(x)= 1/(1+exp(-2bX)) with b=0.5
     * @param v - vector to be squashed
     * @param size - size of vector
     * @param r - squashing result
     */
    public static void squash(final double[] v, final int size, final double[] r)
    {
    ...
    /**
     * Matrix division - divides a M by X
     * @param M - the matrix
     * @param im - matrix width
     * @param jm - matrix height
     * @param X - scalar
     */
    public static void divideBy(final double[][] M, final int im, final int jm, final double X)
    {
        for (int i=0;i<im;i++)
            for (int j=0;j<jm;j++)
                M[i][j] /= X;
    ...
    public static void divideBy(final double[] M, final int m, final double X)
    {
        for(int i=0; i<m; i++)
            M[i]/=X;
    }
